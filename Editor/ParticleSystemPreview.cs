...
namespace mitaywalle.ParticleSystemPrefabPreview.Editor
{
	...
	{
		const string AutoPlayKey = "PSPreviewAutoPlay";
		...
		Vector2 m_PreviewDir = new Vector2(120f, -20f);
		Mesh m_FloorPlane;
		...
		bool m_Loaded;
		int m_PreviewHint = "Preview".GetHashCode();
		int m_PreviewSceneHint = "PreviewSene".GetHashCode();

		float m_RunningTime;
		double m_PreviousTime;
		float m_PlaybackSpeed = 1f;
		bool m_LockParticleSystem = true;
		bool m_HasPreview;
		UnityEditor.Editor m_CacheEditor;
		const float kDuration = 99f;
		static int PreviewCullingLayer = 31;
		...
		void InitPreview()
		{
			...
			bool flag = EditorPrefs.GetBool(AutoPlayKey);
			if (flag != m_Playing)
			{
				if (flag)
				{
		...
		void AddSingleGO(GameObject go)
		{
#if UNITY_2017_1_OR_NEWER
		...
		bool SetupPreviewLightingAndFx()
		{
#if UNITY_2017_1_OR_NEWER
		...
		static void TeardownPreviewLightingAndFx(bool oldFog)
		{
		...
		void PositionPreviewObjects(Quaternion pivotRot, Vector3 pivotPos, Quaternion bodyRot, Vector3 bodyPos,
		...
		void SimulateEnable()
		{
			...
			m_RunningTime = 0f;
			m_Playing = true;
			EditorPrefs.SetBool(AutoPlayKey, true);
		}
		...
		void SimulateDisableClick()
		{
			EditorPrefs.SetBool(AutoPlayKey, false);
			SimulateDisable();
		}
		...
		static int CycleButton(int selected, GUIContent[] contents, GUIStyle style)
		{
